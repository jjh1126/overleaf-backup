...
itself, is $\delta(\{z\})\setminus\{e\}$. If the total initial capacity of these links,
$\sum_{e'\in\delta(\{z\})\setminus\{e\}}c_{e'}$, is positive, its endpoint connectivity is defined as
the residual-to-initial capacity ratio of these links,
$\sum_{e'\in\delta(\{z\})\setminus\{e\}}r_{e'}\big/\sum_{e'\in\delta(\{z\})\setminus\{e\}}c_{e'}$. If
that total initial capacity is zero, endpoint $z$ has no external link resource through which $e$
can be extended; this endpoint is excluded from the link-connectivity comparison rather than
...
The connectivity-deficiency increments in the above expressions are non-negative, so the induced
fragmentation cost is non-negative by construction. This follows from the connectivity monotonicity
property established in Section~\ref{sec:metric_definition_of_resource_fragmentation}: because an
embedding only consumes substrate resources and removes bandwidth from the associated boundary, the
external connectivity of an eligible node or link cannot increase, i.e.,
$I_N(v;S(M))-I_N(v;S_0)\ge0$ for every $v\in N^S$ and $I_L(e;S(M))-I_L(e;S_0)\ge0$ for every $e\in
L^S$. The residual-resource factors ensure that the metric penalizes degradation in the usability of
resources that remain available after the embedding, rather than merely charging the resources that
...
\label{eq:objective}
\end{equation}
where the induced link and node fragmentation terms are
\begin{equation}
\Delta F_L(M)
=
\sum_{e\in L^S}
...
\textbf{Remarks:}
\begin{itemize}
    \item The objective~\eqref{eq:objective} is exactly the induced fragmentation cost
    $\Delta F_{\mathrm{frag}}(M)$ introduced in
    Section~\ref{sec:metric_definition_of_resource_fragmentation}: $\beta$ controls the contribution
    of link fragmentation~\eqref{eq:delta_fl} and $\gamma$ controls the contribution of node
    fragmentation~\eqref{eq:delta_fn}, both evaluated relative to the fixed reference state $S_0$.
