...
\begin{abstract}
    ...
    routing. We further decompose the change in
    ...
    baselines and 2.6 times over the load-balancing baselines.
...
of the same VNR are compared on a common basis. Because the external connectivity of a node or a link is evaluated
from the residual bandwidth of its adjacent links alone, the
fragmentation metric can be evaluated using local residual-bandwidth
information.
...
capacity of the boundary cut between a resource and its exterior and is monotone under embedding. Second, we separate induced fragmentation from the direct effect of resource
...
and its associated hyperparameter. Their
...
induced fragmentation cost they impose on the substrate network. Let
$J_{\mathrm{ECF}}(M) = \beta\,\Delta F_L(M) + \gamma\,\Delta F_N(M)$,
where $\Delta F_L(M)$ and $\Delta F_N(M)$ are the induced link and node
fragmentation defined in \eqref{eq:delta_fl} and \eqref{eq:delta_fn}, both
evaluated relative to the fixed reference state $S_0$, and $\beta,\gamma\ge0$
control the contributions of link and node fragmentation, respectively.
\ecfvne{} then solves
\begin{equation}
...
\ecfvne{}, both weighting factors of the objective are set to $\beta=\gamma=1$.
...
We evaluate each algorithm over the measurement interval
$\mathcal{T}=[t_0,t_1]$ of duration $T=t_1-t_0$.
Let $\mathcal{R}$ denote the set of VNRs arriving during
$\mathcal{T}$, and let $a_r\in\{0,1\}$ indicate whether VNR $r$
is accepted. For an accepted VNR $r$, let $N_r$ and $L_r$ denote
its virtual-node and virtual-link sets, respectively. The CPU demand
of virtual node $n\in N_r$ is denoted by $d_n$, and the bandwidth
demand of virtual link $l\in L_r$ is denoted by $d_l$.
Let $P_l$ be the substrate path used to embed virtual link $l$,
and let $|P_l|$ denote its hop count.

\subsubsection{VNR Acceptance Ratio}
The VNR acceptance ratio is defined as
\begin{equation}
\mathrm{AR}
=
\frac{\sum_{r\in\mathcal{R}} a_r}
{|\mathcal{R}|}.
\label{eq:acceptance-ratio}
\end{equation}
This metric measures the fraction of arriving VNRs that can be
successfully embedded while satisfying all CPU and bandwidth
constraints.

\subsubsection{Revenue}
The revenue of an accepted VNR is defined as the total amount of
virtual resources requested:
\begin{equation}
\mathrm{Rev}(r)
=
\sum_{n\in N_r} d_n
...
\end{equation}
The time-average revenue rate is then
\begin{equation}
\overline{\mathrm{Rev}}
=
\frac{1}{T}
\sum_{r\in\mathcal{R}} a_r\,\mathrm{Rev}(r).
\label{eq:revenue-rate}
\end{equation}
A higher value indicates that the algorithm accommodates a larger
amount of virtual-resource demand over time.

\subsubsection{Embedding Cost}
The embedding cost of an accepted VNR accounts for the CPU allocated
to its virtual nodes and for the bandwidth consumed on every
substrate link traversed by its virtual links:
\begin{equation}
\mathrm{Cost}(r)
=
\sum_{n\in N_r} d_n
+
\sum_{l\in L_r} d_l |P_l|.
\label{eq:embedding-cost}
\end{equation}
Equivalently, if $f_e^l$ denotes the bandwidth allocated to virtual
link $l$ on substrate link $e$, the link-related term can be written
as $\sum_{e\in L_S}\sum_{l\in L_r} f_e^l$.
The average embedding cost per accepted VNR is
\begin{equation}
\overline{\mathrm{Cost}}
=
\frac{\sum_{r\in\mathcal{R}} a_r\,\mathrm{Cost}(r)}
{\sum_{r\in\mathcal{R}} a_r}.
\label{eq:average-embedding-cost}
\end{equation}
This metric penalizes long substrate paths because the bandwidth
demand of a virtual link is reserved on each substrate link along
its path.

\subsubsection{Revenue-to-Cost Ratio}
To assess resource efficiency in addition to absolute admitted
demand, we report the revenue-to-cost ratio:
\begin{equation}
\mathrm{R/C}
=
\frac{\sum_{r\in\mathcal{R}} a_r\,\mathrm{Rev}(r)}
{\sum_{r\in\mathcal{R}} a_r\,\mathrm{Cost}(r)}.
\label{eq:revenue-cost-ratio}
\end{equation}
A larger value indicates that the accepted virtual resources are
provisioned with less substrate-resource consumption, especially
less path-stretch-induced bandwidth consumption.

\subsubsection{Substrate Resource Utilization}
Let $c_v$ and $r_v(t)$ denote the initial and residual CPU capacities
of substrate node $v$, respectively. Likewise, let $c_e$ and
$r_e(t)$ denote the initial and residual bandwidth capacities of
substrate link $e$. The time-average node and link utilizations are
defined as
\begin{equation}
\overline{U}_{\mathrm{node}}
=
\frac{1}{T}
\int_{t_0}^{t_1}
\frac{\sum_{v\in N_S}\left(c_v-r_v(t)\right)}
{\sum_{v\in N_S}c_v}
\,dt,
\label{eq:node-utilization}
\end{equation}
and
\begin{equation}
\overline{U}_{\mathrm{link}}
=
\frac{1}{T}
\int_{t_0}^{t_1}
\frac{\sum_{e\in L_S}\left(c_e-r_e(t)\right)}
{\sum_{e\in L_S}c_e}
\,dt,
\label{eq:link-utilization}
\end{equation}
respectively.
These metrics quantify the fraction of substrate CPU and bandwidth
capacity occupied on average during the measurement interval.
...
Several directions remain for future work. The proposed external connectivity is evaluated only through the boundary formed by the links immediately adjacent to a single node or to the two endpoints of a single link. Consequently, it cannot detect a shared bottleneck that lies deeper in the substrate, where multiple future virtual links may need to traverse the same interior link or narrow cut even though each resource's immediate boundary appears well connected. This limitation reflects the fact that the boundary link set used in this paper is a particular, local instance of the cut in the max-flow min-cut sense, while the actual bottleneck separating a resource region from the rest of the substrate may correspond to a deeper cut. A natural extension is therefore to replace the local boundary capacity with the max-flow, or equivalently the global min-cut, between a resource region and the rest of the substrate network, so that the connectivity term reflects the true reachable capacity rather than only its immediate surroundings. Since computing an exact max-flow for every candidate substrate node evaluated during the greedy embedding process would substantially increase the per-request running time and undermine the low-latency behavior of the current algorithm, this extension should be combined with techniques that keep the computation tractable, such as random-sampling-based approximate min-cut, or incremental max-flow algorithms that reuse previous flow computations and update only the portion of the substrate affected by the most recent embedding. Realizing such an approximate or incremental max-flow-based connectivity term, without sacrificing the locality and low computational cost that make the current metric suitable for online embedding, is left as a direction for future work.
