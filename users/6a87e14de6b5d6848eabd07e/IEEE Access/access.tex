...
\COMMENT{fixed reference state for this VNR}
...
\COMMENT{place the root virtual node}
\IF{$v_r=\emptyset$}
...
\FOR{each virtual node $n\in\pi\setminus\{r\}$}
    \COMMENT{enumerate candidate substrate nodes for $n$}
    ...
    \FOR{each substrate node $v\in N^S$}
        ...
        \COMMENT{route pending virtual links to mapped neighbors}
    ...
    \COMMENT{no feasible candidate for $n$}
    ...
    \COMMENT{commit the minimum-cost candidate}
\ENDFOR
...
\FOR{each substrate node $v\in N^S$}
    ...
    \COMMENT{minimize isolation; break ties by larger residual CPU, then smaller index}
...
maximizes the tuple $\bigl(\deg(n),\,d_n,\,\sum_{l\in L^V:n\in l}d_l\bigr)$, with the final tie
resolved by the smaller virtual-node index. This choice prioritizes virtual nodes that have more
incident constraints and larger resource requirements. Starting from the selected root, the
remaining virtual nodes are processed in breadth-first order. If the VNR contains more than one
connected component, the traversal resumes from the smallest-index unvisited virtual node after
...
For each virtual node, ECF-VNE examines at most $|N^S|$ substrate-node candidates, so the aggregate
