...
For a substrate link $e=\{u,v\}\in L^S$, we evaluate the residual
bandwidth of $e$ according to the external bandwidth resources available at
its endpoints. For each endpoint $z\in\{u,v\}$, define the external
neighboring-link set, excluding $e$ itself, as
\[
\mathcal{N}_{L,z}(e)
=
\delta(\{z\})\setminus\{e\}.
\]
The corresponding initial and residual external bandwidth capacities are
\[
B_{\mathrm{cap},z}^{L}(e)
=
\sum_{q\in\mathcal{N}_{L,z}(e)}c_q,
\qquad
B_{\mathrm{res},z}^{L}(e)
=
\sum_{q\in\mathcal{N}_{L,z}(e)}r_q.
\]

Only endpoints with a positive initial external bandwidth capacity
participate in the bottleneck calculation. Let
\[
\mathcal{A}_L(e)
=
\left\{
z\in\{u,v\}
\mid
B_{\mathrm{cap},z}^{L}(e)>0
\right\}.
\]
For every $z\in\mathcal{A}_L(e)$, its endpoint connectivity is
\[
\mathrm{Conn}_{L,z}(e)
=
\frac{
...
The link external connectivity is defined as
\[
\mathrm{Conn}_L(e)
=
\begin{cases}
\displaystyle
\min_{z\in\mathcal{A}_L(e)}
\mathrm{Conn}_{L,z}(e),
&
\mathcal{A}_L(e)\neq\emptyset,
\\[3mm]
0,
&
\mathcal{A}_L(e)=\emptyset.
\end{cases}
\]
Accordingly, the link connectivity-deficiency factor and fragmentation
score are
\[
...
When both endpoints have external neighboring links, the less connected
endpoint determines the usability of $e$. When exactly one endpoint has
external neighboring links, only that endpoint participates in the
calculation. When neither endpoint has an external neighboring link,
$\mathrm{Conn}_L(e)=0$ and $I_L(e)=1$, indicating that the residual
bandwidth of $e$ is completely isolated from the rest of the substrate
network.
