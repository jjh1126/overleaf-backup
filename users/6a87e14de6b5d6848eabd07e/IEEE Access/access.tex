...
We propose ECF-VNE, a sequential greedy algorithm that evaluates each
partial embedding relative to the fixed residual state $S_0$ observed when
the current VNR arrives. The algorithm first selects a root virtual node by
lexicographically maximizing its degree, CPU demand, and total incident
virtual-link bandwidth; ties are resolved by the smaller virtual-node index.
It then processes virtual nodes in breadth-first order from that root. If a
VNR has disconnected components, the traversal resumes at the smallest-index
unvisited virtual node after the preceding component is exhausted.

Algorithm~\ref{alg:ecf_main} presents the main greedy procedure.
\textsc{RootPlacement}, given in Algorithm~\ref{alg:ecf_root}, selects a
CPU-feasible substrate node with minimum node isolation for the root. It
breaks ties by larger residual CPU and then by the smaller substrate-node
index. For each later virtual node, the main procedure tentatively maps that
node to every unused CPU-feasible substrate node. It then routes all virtual
links to already mapped neighbors in increasing virtual-link index order.
Each link is assigned one bandwidth-feasible minimum-hop substrate path found
by breadth-first search. A substrate-node candidate is discarded if any such
path does not exist. Otherwise, the complete tentative partial embedding is
scored by $J_{\mathrm{ECF}}$, and the minimum-score candidate is committed.
Equal scores are resolved by the smaller substrate-node index.

\begin{algorithm}[!t]
\caption{Main Greedy ECF-VNE Procedure}
\label{alg:ecf_main}
\footnotesize
\begin{algorithmic}[1]
\STATE \textbf{Input:} substrate network $G^S$, VNR $G^V$, weight $\beta$
...
\STATE Obtain BFS order $\pi$ from $r$
\STATE $M\leftarrow\emptyset$
\STATE $v_r\leftarrow\textsc{RootPlacement}(G^S,G^V,r)$
\IF{$v_r=\emptyset$}
    \STATE \textbf{reject due to CPU infeasibility}; \textbf{return} $\emptyset$
\ENDIF
...
\FOR{each virtual node $n\in\pi$, in order}
    ...
    \FOR{each substrate node $v\in\mathcal{V}$}
        ...
        \FOR{each $l=\{n,m\}\in\mathcal{L}_n$, in increasing link-index order}
            \STATE $p\leftarrow$ bandwidth-feasible minimum-hop path from
...
\begin{algorithm}[!t]
\caption{\textsc{RootPlacement}$(G^S,G^V,r)$}
\label{alg:ecf_root}
\footnotesize
\begin{algorithmic}[1]
\STATE \textbf{Input:} current substrate network $G^S$, VNR $G^V$, root $r$
\STATE \textbf{Output:} root substrate node $v_r$ or $\emptyset$
\STATE $v_r\leftarrow\emptyset$
\STATE $I^*\leftarrow+\infty$, $R^*\leftarrow-\infty$
\FOR{each substrate node $v\in N^S$ in increasing index order}
    \IF{$r_v<d_r$}
        \STATE \textbf{continue}
    \ENDIF
    \STATE $I\leftarrow I_N(v)$
    \IF{$I<I^*$, or $I=I^*$ and $r_v>R^*$}
        \STATE $v_r\leftarrow v$, $I^*\leftarrow I$, $R^*\leftarrow r_v$
    \ENDIF
\ENDFOR
...
The algorithm never maps two virtual nodes of the same request to one
substrate node. Hence, every accepted request satisfies
$|N^V|\le |N^S|$. Write $n_s=|N^S|$, $m_s=|L^S|$,
$n_v=|N^V|$, and $m_v=|L^V|$. During processing of one request, at most
$n_s$ substrate-node candidates are evaluated for each of the $n_v$ virtual
nodes. Each virtual link is routed exactly once, when its second endpoint is
mapped. A bandwidth-feasible breadth-first search costs $O(n_s+m_s)$, while
an ECF objective evaluation costs $O(n_s+m_s)$ when node and link
connectivities are accumulated by scans of substrate adjacency lists.

Consequently, the worst-case time complexity per request is
\[
T_{\,\mathrm{ECF\text{-}VNE}}
=
O\!\left(
 n_s n_v
+
...
\right).
\]
The first term accounts for candidate enumeration, the second for
minimum-hop path searches, and the third for objective evaluations. Since
$n_v\le n_s$ and $m_v\le O(n_v^2)\le O(n_s^2)$, the bound can be specialized
according to substrate density.

For a sparse substrate network with $m_s=O(n_s)$,
\[
T_{\mathrm{sparse}}
=
O\!\left(n_s^2m_v+n_s^2n_v\right)
=
O\!\left(n_s^2(m_v+n_v)\right).
\]
For a dense substrate network with $m_s=O(n_s^2)$,
\[
T_{\mathrm{dense}}
=
O\!\left(n_s^3m_v+n_s^3n_v\right)
=
O\!\left(n_s^3(m_v+n_v)\right).
\]
Under the additional worst-case condition of a dense VNR,
$m_v=O(n_v^2)$ and $n_v=\Theta(n_s)$, these simplify to
$T_{\mathrm{sparse}}=O(n_s^4)$ and
$T_{\mathrm{dense}}=O(n_s^5)$, respectively.
