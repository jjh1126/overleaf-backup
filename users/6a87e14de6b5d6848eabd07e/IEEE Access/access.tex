...
We evaluate the VNR acceptance ratio, average revenue rate, average
embedding cost, and average node and link utilization. The definitions
of the VNR acceptance ratio, VNR revenue, and embedding cost follow
the standard VNE evaluation metrics in~\cite{zhang2025adaptive}.

For an accepted VNR $r\in\mathcal{R}_{\mathrm{acc}}(T)$, let
$\mathrm{Rev}(r)$ and $\mathrm{Cost}(r)$ denote its revenue and
embedding cost, respectively. The long-term average revenue rate is
defined as
\begin{equation}
	\lim_{T\to\infty}
	\frac{
		\sum_{r\in\mathcal{R}_{\mathrm{acc}}(T)}
		\mathrm{Rev}(r)
		}{T}.
	\label{eq:average-revenue-rate}
\end{equation}

The average embedding cost per accepted VNR is defined as
\begin{equation}
	\lim_{T\to\infty}
...
Finally, average node utilization and average link utilization are
used to measure the time-average fractions of consumed substrate CPU
and bandwidth capacities, respectively.
