...
The objective of \ecfvne{} is to embed the current VNR while preserving
the future usability of the residual substrate resources. In the proposed
method, the quality of a tentative embedding is evaluated by the
connectivity degradation that it induces in the residual substrate network.

The decision variable $x_{nv}\in\{0,1\}$ indicates whether virtual node
$n\in N^V$ is mapped to substrate node $v\in N^S$, and
$f_{uv}^l\ge0$ denotes the bandwidth of virtual link $l$ routed through
substrate arc $(u,v)\in A^S$.

\subsection{Embedding Constraints}

Each virtual node must be mapped to exactly one substrate node. In addition,
distinct virtual nodes belonging to the same VNR cannot be mapped to the
same substrate node:
\[
...
The total CPU demand allocated to each substrate node must not exceed its
...
substrate nodes serving as its source and destination:
\[
\begin{aligned}
...
The parameter $\gamma$ controls the relative contribution of node
fragmentation. Thus, a candidate embedding is preferred when it produces a
smaller degradation in the external connectivity of the residual substrate
resources relative to the state observed at the arrival of the current VNR.


\section{ECF-VNE Embedding Algorithm}
\label{sec:embedding_algorithm}

We now present the ECF-VNE algorithm, which constructs an embedding
sequentially and greedily. For each arriving VNR, the current residual
substrate state is stored as the reference state $S_0$. Every tentative
partial embedding of that VNR is evaluated relative to this same reference
state using $J_{\mathrm{ECF}}$.

The algorithm first selects a root virtual node by lexicographically
maximizing its degree, CPU demand, and the total bandwidth demand of its
incident virtual links. Ties are broken by the smaller virtual-node index.
The remaining virtual nodes are then processed in breadth-first order from
the selected root. If the VNR contains disconnected components, breadth-first
traversal continues from the smallest-index unvisited virtual node after one
component has been processed.

Before mapping the root, the algorithm considers all substrate nodes with
sufficient residual CPU. It selects the node with the minimum node isolation.
Ties are resolved by choosing the node with larger residual CPU capacity and
then the smaller substrate-node index.

For each subsequent virtual node $n$, the algorithm examines every substrate
node $v$ that has sufficient residual CPU and has not already been assigned
to another virtual node of the same VNR. For each candidate $v$, a tentative
embedding $\widehat{M}=M\cup\{n\mapsto v\}$ is formed. The virtual links
between $n$ and its already mapped neighbors are then routed in increasing
virtual-link index order.

For each required virtual link $l=\{n,m\}$, the algorithm finds one feasible
minimum-hop substrate path from $v$ to $M_N(m)$ by breadth-first search. Only
substrate links with residual bandwidth sufficient for the demand of $l$ are
traversed. When multiple minimum-hop paths exist, the breadth-first search
uses a deterministic order based on neighboring substrate-node indices and
then substrate-link indices. The bandwidth demand of $l$ is allocated on the
selected path before the next pending virtual link is processed.

If no feasible path exists for any required virtual link, the corresponding
substrate-node candidate is discarded. Otherwise, the algorithm evaluates
the complete tentative partial embedding using $J_{\mathrm{ECF}}$. After all
feasible substrate-node candidates have been evaluated, the candidate with
the smallest objective value is committed. Equal objective values are resolved
by the smaller substrate-node index. The algorithm does not revise previously
committed mapping decisions.

If no unused substrate node has sufficient residual CPU for the current
virtual node, the VNR is rejected due to CPU infeasibility. If one or more
CPU-feasible substrate nodes exist but none can route all required virtual
links, the VNR is rejected due to routing infeasibility.

\begin{algorithm}[!t]
\caption{ECF-VNE Algorithm}
\label{alg:ecf_algo}
\footnotesize
\begin{algorithmic}[1]
\STATE \textbf{Input:} substrate network $G^S$, VNR $G^V$, weight $\gamma$
...
\STATE Obtain the BFS order $\pi$ from $r$
\STATE $M\leftarrow\emptyset$
\STATE Select root substrate node $v_r$ with minimum node isolation;
break ties by larger residual CPU and then smaller index
...
\FOR{each virtual node $n\in\pi$, in order}
    ...
    \IF{$n=r$}
        \STATE $\mathcal{V}\leftarrow\{v_r\}$
    \ELSE
        \STATE $\mathcal{V}\leftarrow N^S$
    \ENDIF

    \FOR{each substrate node $v\in\mathcal{V}$}
        \IF{$v$ is already used by $M_N$ or $r_v<d_n$}
        ...
        \{l=\{n,m\}\in L^V:m\text{ is already mapped}\}$

        \FOR{each $l=\{n,m\}\in\mathcal{L}_n$, in increasing link-index order}
            \STATE $p\leftarrow$ feasible minimum-hop path from
            $v$ to $M_N(m)$ under $\widehat{M}$
    ...
    \IF{$\mathcal{C}=\emptyset$}
        \IF{\textbf{not} $\mathit{hasCpuCandidate}$}
            \STATE \textbf{reject due to CPU infeasibility}
        \ELSE
            \STATE \textbf{reject due to routing infeasibility}
        \ENDIF
        \STATE \textbf{return} $\emptyset$
    \ENDIF

    \STATE $(M,\cdot)\leftarrow
    \arg\min_{(\widehat{M},J_{\mathrm{ECF}}(\widehat{M}))\in\mathcal{C}}
    J_{\mathrm{ECF}}(\widehat{M})$
\ENDFOR
...
For every non-root virtual node, ECF-VNE examines at most $|N^S|$
substrate-node candidates. Each virtual link is routed once, when the second
endpoint is mapped. For a candidate mapping, each required virtual link is
routed by a breadth-first search over bandwidth-feasible substrate links.
The path-search cost is therefore $O(|N^S|+|L^S|)$ per routed virtual link.

The ECF objective is evaluated once for every feasible substrate-node
candidate. Node isolation can be computed by scanning the incident substrate
links of every substrate node. Link isolation is computed from the external
neighboring links of its endpoints; endpoints without external neighboring
resources are excluded from the bottleneck comparison. Hence, an objective
evaluation requires $O(|L^S|)$ time when the required incident-capacity sums
are accumulated during a scan of the substrate links.

Accordingly, a worst-case upper bound for the greedy embedding procedure is
\[
O\left(
|N^S||N^V|
+
...
\right).
\]
The first term represents candidate enumeration, the second represents
bandwidth-feasible minimum-hop path searches, and the third represents ECF
objective evaluations for tentative substrate-node candidates.
