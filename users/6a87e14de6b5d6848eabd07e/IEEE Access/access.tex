...
We evaluate the following performance metrics over the measurement period
$\mathcal{T}=[t_0,t_1]$ with duration $T=t_1-t_0$. Let $\mathcal{R}$
be the set of VNRs arriving during $\mathcal{T}$, and let
$a_r\in\{0,1\}$ indicate whether VNR $r$ is accepted.

\begin{itemize}
    \item \textit{VNR acceptance ratio:}
    \begin{equation}
    \mathrm{AR}=
    \frac{\sum_{r\in\mathcal{R}} a_r}{|\mathcal{R}|}.
    \end{equation}

    \item \textit{Average revenue rate:}
    The revenue of VNR $r$ is
    \begin{equation}
    \mathrm{Rev}(r)=
    \sum_{n\in N_r} d_n+
    \sum_{l\in L_r} d_l,
    \end{equation}
    and the average revenue rate is
    \begin{equation}
    \overline{\mathrm{Rev}}=
    \frac{1}{T}\sum_{r\in\mathcal{R}}a_r\,\mathrm{Rev}(r).
    \end{equation}

    \item \textit{Average embedding cost:}
    Let $P_l$ denote the substrate path used for virtual link $l$.
    The embedding cost of an accepted VNR $r$ is
    \begin{equation}
    \mathrm{Cost}(r)=
    \sum_{n\in N_r} d_n+
    \sum_{l\in L_r} d_l |P_l|,
    \end{equation}
    and the average embedding cost per accepted VNR is
    \begin{equation}
    ...
    \item \textit{Average node utilization:}
    \begin{equation}
    \overline{U}_{\mathrm{node}}=
    \frac{1}{T}\int_{t_0}^{t_1}
    \frac{\sum_{v\in N_S}(c_v-r_v(t))}
    {\sum_{v\in N_S}c_v}\,dt.
    \end{equation}

    \item \textit{Average link utilization:}
    \begin{equation}
    \overline{U}_{\mathrm{link}}=
    \frac{1}{T}\int_{t_0}^{t_1}
    \frac{\sum_{e\in L_S}(c_e-r_e(t))}
    {\sum_{e\in L_S}c_e}\,dt.
    \end{equation}
\end{itemize}
