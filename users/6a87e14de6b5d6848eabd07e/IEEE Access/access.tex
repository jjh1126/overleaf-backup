...
separates the two regions coincides with $\delta(A)$; the boundary's residual capacity therefore
represents the capacity available for region $A$ to connect with its exterior.
...
single-commodity flow from $A$ to $\bar A$ cannot exceed $\sum_{e\in\delta(A)}r_e$. Thus, if
...
$\mathrm{Conn}(A;M)\le\mathrm{Conn}(A)$. Consequently $I(A;M)-I(A)\ge0$ for every $A$, so the induced
fragmentation cost is non-negative by construction. These two properties keep the metric
interpretable as a resource-usability bound and ensure that the induced fragmentation cost behaves
consistently under embedding.
