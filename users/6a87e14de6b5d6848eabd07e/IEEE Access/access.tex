...
for substrate nodes and links. The external connectivity term is justified directly by the boundary
cut, equivalently the maximum flow between a resource region and its exterior, and is monotone
under embedding, which distinguishes it from hop-bounded neighborhood aggregates that depend on
tuning parameters. Second, we separate induced fragmentation from the direct effect of resource
...
BCube data center topologies under a range of arrival rates. The results show that \ecfvne{}
...
The external connectivity of $e$ is determined by the minimum endpoint connectivity among the
endpoints that possess external link resources:
\begin{equation}
    \mathrm{Conn}_L(e)
    =
    \begin{cases}
        \displaystyle\min_{\substack{z\in\{u,v\}\\ \sum_{e'\in\delta(\{z\})\setminus\{e\}}c_{e'}>0}}
        \mathrm{Conn}_z(e),
         & \text{if such $z$ exists}, \\[3mm]
        0,
         & \text{otherwise.}
    \end{cases}
\label{eq:conn_l}
...
rather than merely charging the resources that have already been consumed.
...
Section~\ref{sec:related_work}; the two variants differ in the search strategy used for node mapping. The
...
resources of a hop-bounded neighborhood: in a DCN, the neighbors of every host are switches with
zero CPU capacity, the topology is regular, and the link capacities are uniform, so the aggregated
neighborhood states of different hosts become nearly indistinguishable and the metric provides
little guidance for node selection. In contrast, \ecfvne{} preserves its advantage on the DCNs
...
and its exterior, which gives it a max-flow interpretation and guarantees that it is monotone under
...
algorithms on every topology and at every tested arrival rate. In particular, whereas the existing
